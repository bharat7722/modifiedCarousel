...
const FirstSlide = (props) => {
  ...
  const [carouselType, setcarouselType] = useState("twitter")
  ...
  const handleSocialIcon = (e) => {
    ...
    const fileReader = new FileReader();
    fileReader.onload = () => {
      setsocialIcon(fileReader.result);
    };
    fileReader.readAsDataURL(file);
  };
    ...
    <div className="slide-wrapper">
      ...
      >
        ...
        <div className="slide-content">
          <div
            className="twitter__post--container first__Slide--item"
            style={{ boxShadow: `10px 10px 0px ${colorPalate.third}` }}
          >
            <div className="post__container--header">
              <div className="userDetails__box">
                ...
                <div className="userText">
                  {name.status && (
                    <div className={` name name-font-${textSize}`} style={{ fontFamily: fontFamily,fontSize: textSize === "small" ? "12px" : textSize === "medium" ? "14px" : "16px" }}>
                      {name.value}
                    </div>
                  )}
                  {userName.status && (
                    <div
                      className="userName"
                      style={{ fontFamily: fontFamily, fontSize: textSize === "small" ? "12px" : textSize === "medium" ? "14px" : "16px" }}
                    >
                      {userName.value}
                    </div>
                  )}
            ...
            <div className="post__container--body">
              {content.status && (
                <p style={{ fontFamily: fontFamily, textAlign: alignment }} className={`title description-font-${textSize}`}>{content.value}</p>
              )}
            </div>
          </div>
        </div>
